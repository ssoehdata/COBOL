      ...
      *--------------------------------
       ...
       01  TITLE-LINE. 
           ...
           05 FORMATTED-RUN-DATE           PIC X(10). 
           05 FILLER                       PIC X(10) VALUE "AT".
           05 FORMATTED-RUN-TIME           PIC X(8). 
       ...
       01  TOTAL-LINE.
           ...
           05 FILLER                       PIC X(1) VALUE SPACE. 
           05 TOTAL-LITERAL                PIC X(5) VALUE SPACE. 
           05 FILLER                       PIC X(1) VALUE SPACE. 
      ...
      * CONTROL BREAK CURRENT VALUES FOR STORE, DIVISION, DEPARTMENT. 
      ...
      *-----------------------------------------------
      * Fields for TIME routines.
      *-----------------------------------------------
       77  FORMATTED-TIME                  PIC Z9/99/9999.
       ...
       1000-PROGRAM-BEGIN. 
           ...
           PERFORM 1300-OPENING-PROCEDURE.
           PERFORM 2000-MAIN-PROCESS.
           PERFORM 1400-CLOSING-PROCEDURE.
       ...
       1300-OPENING-PROCEDURE.

           OPEN OUTPUT PRINTER-FILE.

       1400-CLOSING-PROCEDURE.
           CLOSE PRINTER-FILE.

       2000-MAIN-PROCESS.
           PERFORM 2100-GET-OK-TO-PROCESS.
           PERFORM 3000-PROCESS-THE-FILE 
               UNTIL OK-TO-PROCESS = "N".

       2100-GET-OK-TO-PROCESS.
           PERFORM 2110-ACCEPT-OK-TO-PROCESS.
           PERFORM 2120-RE-ACCEPT-OK-TO-PROCESS 
               UNTIL OK-TO-PROCESS = "Y" OR "N".

       2110-ACCEPT-OK-TO-PROCESS.
       ...
       2120-RE-ACCEPT-OK-TO-PROCESS.
           DISPLAY "YOU MUST ENTER YES OR NO".
           PERFORM 2110-ACCEPT-OK-TO-PROCESS.

       3000-PROCESS-THE-FILE. 
           PERFORM 3100-START-THE-FILE. 
           PERFORM 4000-PRINT-ONE-REPORT.
           PERFORM 3200-END-THE-FILE.

           MOVE "N" TO OK-TO-PROCESS.

       3100-START-THE-FILE.
           PERFORM 3110-SORT-DATA-FILE.
      ...
      * LEVEL 1 CONTROL BREAK 
       4000-PRINT-ONE-REPORT.
           PERFORM 4100-START-ONE-REPORT.
           PERFORM 5000-PROCESS-ALL-STORES 
               UNTIL WORK-FILE-AT-END = "Y".
           PERFORM 4200-END-ONE-REPORT. 

       4100-START-ONE-REPORT.
           PERFORM READ-FIRST-VALID-WORK. 
           MOVE ZEROES TO GRAND-TOTAL. 

           PERFORM 4110-START-NEW-REPORT.

       4110-START-NEW-REPORT.
           ...
           MOVE FORMATTED-DATE TO FORMATTED-RUN-DATE.

           ACCEPT RUN-TIME FROM TIME.
           COMPUTE TIME-HHMMSS = RUN-TIME / 100.
           PERFORM FORMAT-THE-TIME.
           MOVE FORMATTED-TIME TO FORMATTED-RUN-TIME. 
       ...
       4200-END-ONE-REPORT. 
           ...
           ELSE 
               PERFORM 4210-PRINT-GRAND-TOTAL.

           PERFORM END-LAST-PAGE. 

       4210-PRINT-GRAND-TOTAL.
      ...
      * LEVEL 2 CONTROL BREAK 
       5000-PROCESS-ALL-STORES.
           PERFORM 5100-START-ONE-STORE. 

           PERFORM 6000-PROCESS-ALL-DIVISIONS 
               UNTIL WORK-FILE-AT-END = "Y" 
                   OR WORK-STORE NOT = "CURRENT-STORE".

           PERFORM 5200-END-ONE-STORE.

       5100-START-ONE-STORE. 
       ...
       5200-END-ONE-STORE.
           PERFORM 5210-PRINT-STORE-TOTAL.
           ADD STORE-TOTAL TO GRAND-TOTAL.

       5210-PRINT-STORE-TOTAL.
           MOVE SPACE TO TOTAL-LINE.
      ...
      * LEVEL 4 CONTROL BREAK
       7000-PROCESS-ALL-DEPARTMENTS.
           PERFORM 7100-START-ONE-DEPARTMENT.

           PERFORM 8000-PROCESS-ALL-CATEGORIES 
               UNTIL WORK-FILE-AT-END = "Y"
                   OR WORK-STORE NOT = CURRENT-STORE 
                   OR WORK-DIVISION NOT = CURRENT-DIVISION 
                   OR WORK-DEPARTMENT NOT = CURRENT-DEPARTMENT.

           PERFORM 7200-END-ONE-DEPARTMENT. 

       7100-START-ONE-DEPARTMENT. 
           MOVE WORK-DEPARTMENT TO CURRENT-DEPARTMENT. 
           MOVE ZEROES TO DEPARTMENT-TOTAL. 
           MOVE WORK-DEPARTMENT TO PRINT-DEPARTMENT. 

       7200-END-ONE-DEPARTMENT. 
           PERFORM 7210-PRINT-DEPARTMENT-TOTAL.
           ADD DEPARTMENT-TOTAL TO DIVISION-TOTAL. 

       7210-PRINT-DEPARTMENT-TOTAL. 
           MOVE SPACE TO TOTAL-LINE.
      ...
      *    PROCESS ONE RECORD LEVEL 
       8000-PROCESS-ALL-CATEGORIES.
           PERFORM 8100-PROCESS-THIS-CATEGORY.
           ADD WORK-AMOUNT TO DEPARTMENT-TOTAL.
           ADD 1 TO RECORD-COUNT.
           PERFORM READ-NEXT-VALID-WORK.

       8100-PROCESS-THIS-CATEGORY.
           IF LINE-COUNT > MAXIMUM-LINES 
               PERFORM START-NEXT-PAGE.
           PERFORM 8110-PRINT-THE-RECORD.

       8110-PRINT-THE-RECORD.
           MOVE WORK-CATEGORY TO PRINT-CATEGORY.

           PERFORM 8120-LOAD-CATEGORY-NAME.
